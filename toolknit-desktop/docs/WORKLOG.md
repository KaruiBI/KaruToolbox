...
| 普通视频工具 | 基础可用 | 已有格式转换、媒体探测、音频提取和 FFmpeg 基础；尚不是多轨剪辑器。 |
...
| 专业视频创作工程 | 基础开发已启动 | Project v1 schema 已完成；磁盘保存、时间线 UI、撤销栈、音频轨、字幕轨和渲染计划尚未实现。 |
...
- 创作内核 Phase 0：约 20%，已完成工程 schema，其他底座仍待实现。
- 多轨视频编辑 MVP：约 5%，复用了现有 FFmpeg 能力，但编辑器本身尚未形成闭环。
...
当前产品决策是先做可见、可用的创作功能，数据库与用户记忆放到后期。下一项优先做 **T003：独立编辑器模块骨架**，随后推进媒体探测、时间线、片段操作和导出闭环；T002 的工程磁盘持久化在编辑器闭环稳定后补齐。
