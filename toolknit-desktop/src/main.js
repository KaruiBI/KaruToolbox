...
const doAudioClear = () => {
    ...
    setTimeout(() => {
        ...
        if (audioSearchInput) {
            ...
            audioSearchInput.style.display = '';
        }
...
const changeStorageFolder = document.getElementById('changeStorageFolder');
...
function updateStoragePathDisplay(path) {
    ...
    storagePathDisplay.textContent = path || '--';
    storagePathDisplay.title = path || '';
}
if (storagePathDisplay) {
    if (isTauri) {
        import ('@tauri-apps/api/core').then(({ invoke }) => {
            invoke('get_install_config').then((config) => {
                updateStoragePathDisplay(config.install_path);
            }).catch(() => {
...
if (changeStorageFolder) {
    changeStorageFolder.addEventListener('click', async() => {
        if (!isTauri) {
            window.showToast ?.(t('settings.desktopOnly'));
            return;
        }
        try {
            const { open } = await
            ...
            const { invoke } = await
            import ('@tauri-apps/api/core');
            const selected = await open({ directory: true, multiple: false, title: t('settings.chooseFolder') });
            if (typeof selected !== 'string') return;
            const savedPath = await invoke('set_storage_path', { path: selected });
            updateStoragePathDisplay(savedPath);
            window.showToast ?.(t('settings.storagePathUpdated'));
        } catch (e) {
            console.error('Change storage folder failed:', e);
            window.showToast ?.(`${t('settings.storagePathUpdateFailed')} ${e?.message || e}`);
        }
    });
}
...
function closeImageConvertOverlay() {
    ...
    if (imageConvertPlasmaInstance) {
        imageConvertPlasmaInstance();
        imageConvertPlasmaInstance = null;
    }
    ...
    if (isTauri) {
        ...
        try {
            ...
            if (inputPaths.length === 0) {
                imageConvertProcessMask.classList.remove('visible');
                processingImage = false;
                alert(t('common.filePathsNotAvailableShort'));
                return;
            }
...
function closeImageCompressOverlay() {
    ...
    if (imageCompressPlasmaInstance) {
        imageCompressPlasmaInstance();
        imageCompressPlasmaInstance = null;
    }
...
function removeImageCompressFile(index) {
    selectedImageCompressFiles.splice(index, 1);
    renderImageCompressFiles();
}

function clearImageCompressFiles() {
    selectedImageCompressFiles = [];
    renderImageCompressFiles();
}
...
if (imageCompressCta) {
    imageCompressCta.addEventListener('click', async() => {
        if (isTauri) {
            ...
            input.accept = 'image/*';
            input.addEventListener('change', () => {
                addImageCompressFiles(input.files);
                input.value = '';
            });
            input.click();
        }
...
function closeVideoConvertOverlay() {
    ...
    if (videoConvertPlasmaInstance) {
        videoConvertPlasmaInstance();
        videoConvertPlasmaInstance = null;
    }
...
document.querySelectorAll('.audio-list-item[data-tool="video-convert"]').forEach(item => {
    ...
    item.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            openToolWithFfmpegCheck(openVideoConvertOverlay);
        }
    });
});
...
async function analyzeBpmAudioBuffer(arrayBuffer) {
    ...
    try {
        ...
        // Analyze BPM
        const { analyzeFullBuffer } = await
        import ('realtime-bpm-analyzer');
...
async function downloadSingleRotatePage(pageIdx) {
    ...
    try {
        const { PDFDocument, degrees } = await
        import ('pdf-lib');
...
function sauvolaBinarize(data, w, h, windowSize, k) {
    ...
    for (let y = 0; y < h; y++) {
        const y0 = Math.max(0, y - halfWin),
            y1 = Math.min(h - 1, y + halfWin);
        for (let x = 0; x < w; x++) {
            const x0 = Math.max(0, x - halfWin),
                x1 = Math.min(w - 1, x + halfWin);
...
async function exportAiDocPdf() {
    ...
    try {
        ...
        const fontkit = (await
            import ('@pdf-lib/fontkit')).default;
...
function closeAiTableOverlay() {
    ...
    resetAiTableState();
    if (aiTableDitherInstance) {
        aiTableDitherInstance();
        aiTableDitherInstance = null;
    }
}
...
function resetAiTableState() {
    ...
    if (aiTablePreviewScroll) {
        aiTablePreviewScroll.style.display = 'none';
...
4. 如果用户要求图表，必须包含 charts 字段`;
        ...
        try { parsed = JSON.parse(jsonStr); } catch (parseErr) {
            ...
            const fallbackJson = jsonStr.replace(/[\u0000-\u001F\uFEFF\uFFFD]/g, ' ').replace(/\n/g, '\\n');
            try { parsed = JSON.parse(fallbackJson); } catch (e2) { addAiTableChatMsg('ai', t('home.aiTable.parseError')); return; }
        }
...
const aiTableDoughnutPlugin = {
    ...
    afterDatasetsDraw: (chart) => {
        ...
        if (arc0) {
            ...
            ctx.font = "700 18px " + AI_TABLE_FONT;
...
function exportAiTablePng() {
    ...
    try {
        ...
        if (aiTableData.title) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText(aiTableData.title, totalW / 2, y + 20);
            y += headerH;
        }
        // Table header
...
async function exportAiTablePdf() {
    ...
    try {
        ...
        // Charts as images
        for (const { canvas: chartCanvas }
            of aiTableChartCanvases) {
        ...
        if (isTauri) {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `ai_table_${Date.now()}.pdf`;
            a.click();
            URL.revokeObjectURL(url);
        }
    } catch (e) {
...
async function downloadAiTableFile(content, mime, fileName) {
    ...
    try { await saveAiTableBlob(blob, fileName); } catch (e) {
        console.error('[AI Table] CSV export error:', e);
        addAiTableChatMsg('ai', t('home.aiTable.errCsv'));
    }
}
...
document.querySelectorAll('.audio-list-item[data-tool="ai-table"]').forEach(item => {
    ...
    item.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            openToolWithAiCheck(openAiTableOverlay);
        }
    });
});
...
function closeColorExtractorOverlay() {
    ...
    resetColorExtractorState();
    if (colorExtractorDitherInstance) {
        colorExtractorDitherInstance();
        colorExtractorDitherInstance = null;
    }
}
...
function closeTextStatsOverlay() {
    ...
    if (textStatsPlasmaInstance) {
        textStatsPlasmaInstance();
        textStatsPlasmaInstance = null;
    }
}
...
function executeFormat(action, text) {
    ...
    switch (action) {
        ...
        case 'capitalize':
            return text.replace(/([.!?。！？]\s*)([a-z\u4e00-\u9fff])/g, (m, p1, p2) => p1 + p2.toUpperCase()).replace(/^([a-z\u4e00-\u9fff])/, (m, p1) => p1.toUpperCase());
        case 'trimSpaces':
...
function closeTextFormatOverlay() {
    ...
    if (textFormatPlasmaInstance) {
        textFormatPlasmaInstance();
        textFormatPlasmaInstance = null;
    }
}
...
function closeTypingTestOverlay() {
    ...
    typingTestState.isRunning = false;
    if (typingTestDitherInstance) {
        typingTestDitherInstance();
        typingTestDitherInstance = null;
    }
}
